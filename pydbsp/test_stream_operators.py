...
from stream_operators import Delay, Differentiate, Integrate, LiftedDelay, LiftedDifferentiate, LiftedGroupAdd, LiftedGroupNegate, LiftedIntegrate, LiftedStreamElimination, LiftedStreamIntroduction, StreamAddition, step_n_times, step_n_times_and_return, stream_elimination, stream_introduction
...
def test_differentiate() -> None:
    ...
    diffed_list = [ s.group().add(i, s.group().neg(s.inner[idx - 1])) if idx > 0 else 0 for (idx, i) in enumerate(s.inner) ]
...
def test_lifted_integrate() -> None:
    ...
    integrated_s = step_n_times_and_return(operator, s.current_time() + 1)
    integrated_list = [ [ 0, 1, 3, 6, ], [ 2, 5, 9, 14 ], [ 4, 9, 15, 22 ], [ 6, 13, 21, 30 ] ]    
    assert from_stream_of_streams_into_list_of_lists(integrated_s) == integrated_list
    assert integrated_s.current_time() == s.current_time()
